...
export const iterateThroughFilter = (filterType: string, filterElemsStateWithFreq: FilterElem[], callback: FilterElemsCallback): void => {
  cy.get(`[data-test-id^=${filterType}-]`).should('have.length', filterElemsStateWithFreq.length)

  for (let numFilterElemsSelected = 1; numFilterElemsSelected <= filterElemsStateWithFreq.length; numFilterElemsSelected++) {
    for (let i = 0; i + numFilterElemsSelected <= filterElemsStateWithFreq.length; i++) {
      const selectedFilterElems: FilterElem[] = filterElemsStateWithFreq.slice(i, i + numFilterElemsSelected)

      if (numFilterElemsSelected === 1 || i !== 0) {
        cy.get(`[data-test-id=${filterType}-${selectedFilterElems[numFilterElemsSelected - 1].value.replace(/\.| /g, '')}]`).click()
      } else {
      ...
      if (numFilterElemsSelected === 1 || i + numFilterElemsSelected !== filterElemsStateWithFreq.length) {
        cy.get(`[data-test-id=${filterType}-${selectedFilterElems[0].value.replace(/\.| /g, '')}]`).click()
      } else {
        selectedFilterElems.forEach((filterElem) => {
          cy.get(`[data-test-id=${filterType}-${filterElem.value.replace(/\.| /g, '')}]`).click()
        })
